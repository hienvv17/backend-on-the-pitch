  ...
  async getDashboardData(year?: number, month?: number) {
    ...
    const bookingsByDateAndStatus = await this.bookingRepository
      ...
      .andWhere('booking.status IN (:...status)', {
        status: [
          FieldBookingStatus.PAID,
          FieldBookingStatus.CHECK_IN,
          FieldBookingStatus.REFUND,
        ],
      })
    ...
    const bookingsBySportCategory = await this.bookingRepository
      ...
      .innerJoin(
        'sport_fields',
        'sportField',
        'sportField.id = booking.sportFieldId',
      )
      .innerJoin(
        'sport_categories',
        'sportCategory',
        'sportCategory.id = sportField.sportCategoryId',
      )
