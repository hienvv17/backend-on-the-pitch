import { IsNotEmpty, Matches, IsString, IsEmail, IsOptional, IsNumber, IsIn } from 'class-validator';
...
export class GetBookingHistoryDto extends ListAllEntities {
    @ApiProperty()
    @IsValidDate({ message: 'fromDate must be a valid date in YYYY-MM-DD format' })
    @IsOptional()
